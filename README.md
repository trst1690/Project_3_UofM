...
- Amberly: created new coding in JS using Leaflet. This code creates a map with markers for 470 records. Initially, the markers did not appear. After some research, she determined the code did not pull the data in json format and needed to be reformatted to geojson. Amberly converted the json file to a geojson file using Pandas. After creating a geojson file, the markers correctly displayed as they should. The JS code colors the markers based on the type of site designation. Additionally, the JS code creates a list of activities for each site as a popup.
